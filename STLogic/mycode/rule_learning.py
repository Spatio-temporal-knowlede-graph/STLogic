...
class Rule_Learner(object):
    def __init__(self, edges, id2relation, inv_relation_id, dataset, positions=None):
        """
        ...
        Parameters:
            ...
            positions (spatial_context.PositionIndex): entity/landmark positions for
                the STLogic spatial context model (None disables spatial fitting)
    ...
    def estimate_confidence(self, rule, num_samples=500):
        ...
        # STLogic: fit the rule-specific spatial context model over grounding bodies.
        # body = [e0, t0, e1, t1, ..., eN, tN]; head = e0, tail = eN, ref ts = body[-2].
        if self.positions is not None and unique_bodies:
            feats = [
                sc.features(self.positions, body[0], body[-1], body[-2])
                for body in unique_bodies
            ]
            rule["spatial"] = sc.fit_gaussian(feats)  # pooled over all groundings

            # ② type-conditioned: split groundings by the candidate (tail) type so
            # heterogeneous contexts (e.g. tank vs artillery) get sharper distributions.
            by_type = {}
            for body, f in zip(unique_bodies, feats):
                typ = self.positions.type_of(body[-1])
                if typ is not None:
                    by_type.setdefault(typ, []).append(f)
            rule["spatial_by_type"] = {
                t: sc.fit_gaussian(fl) for t, fl in by_type.items()
            }
