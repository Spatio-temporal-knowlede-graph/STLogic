...
def get_candidates(
    ...
    Parameters:
        ...
        test_query_sub (int): test query subject (STLogic spatial fit; None disables)
        positions (spatial_context.PositionIndex): entity/landmark positions
        feature_set (tuple): spatial feature subset to apply (None/() disables)
    ...
    spatial_on = positions is not None and feature_set

    for cand in cands:
        cands_walks = rule_walks[rule_walks[max_entity] == cand]
        # STLogic: spatial fit is candidate/rule specific (independent of score args),
        # multiplied into the per-grounding score before the Noisy-OR aggregation.
        s_fit = 1.0
        if spatial_on:
            feat = sc.features(positions, test_query_sub, cand, test_query_ts)
            cand_type = positions.type_of(cand) if type_cond else None
            eff = sc.effective_spatial(rule, cand_type)
            s_fit = sc.spatial_fit(feat, eff, feature_set, f_min)
        for s in dicts_idx:
            score = score_func(rule, cands_walks, test_query_ts, *args[s]).astype(
                np.float32
            )
            if s_fit != 1.0:
                score = np.float32(score * s_fit)
            try:
